...
#return self.encoding[c]
    ...
    def preapreEncodning(self,letters):
        ...
        for l in letters.keys():
            ...
            count+=1
        return encoding
	def getWorstTree(self, trees):
		id = 0
		minValue = trees[0].root(1)
		for i in range(1,len(trees)):
			if(trees[i].root(1)<minValue):
				id =id
				minValue = trees[i].root(1)
		return id
	def prepareEncoding2(self,letters,text):
		count =len(text)
		trees = [] 
		for l in letters.keys():
			element =tuple(l,letters[l]/float(count))
			trees.append(Node(element,True))
		while(len(trees)>1):
			fId = 
			firstTree = 
